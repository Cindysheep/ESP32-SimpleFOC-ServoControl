...
PIDParams vel_pid = {0.15f, 1, 0, 0.01f}; // 速度环参数

// 添加必要的全局变量
float target_position = 0;
float target_velocity = 10;        // 默认速度10rad/s
float current_status = 0;        // 当前状态参数
const float max_velocity = 250;    // 最大速度限制
const float default_velocity = 10; // 默认速度
const float max_torque = 14;       // 最大扭矩限制，实际是电压限制
...
void doTarget(char *cmd) { command.scalar(&target_position, cmd); model = 'p'; }
void doVelocity(char *cmd) { command.scalar(&target_velocity, cmd); model = 'v'; }
void doTorque(char *cmd) { command.scalar(&target_torque, cmd); model = 't'; }
void doForward(char *cmd) { if (strlen(cmd) > 0) command.scalar(&target_velocity, cmd); model = 'v'; }
void doBackward(char *cmd) { if (strlen(cmd) > 0) command.scalar(&target_velocity, cmd); target_velocity = -target_velocity; model = 'v'; }
